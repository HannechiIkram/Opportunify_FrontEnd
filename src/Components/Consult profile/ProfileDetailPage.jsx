...
function ProfileDetailPage() {
  ...
  const handleFollow = async () => {
    try {
      ...
      // Make a POST request to follow the profile
      await axios.post(`consult/follow/${_id}`, null, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
      // Update following status and profile data
      setIsFollowing(true);
      setProfile((prevProfile) => ({
        ...prevProfile,
        followers: [...prevProfile.followers, prevProfile._id], // Add current user ID here
      }));
    } catch (error) {
